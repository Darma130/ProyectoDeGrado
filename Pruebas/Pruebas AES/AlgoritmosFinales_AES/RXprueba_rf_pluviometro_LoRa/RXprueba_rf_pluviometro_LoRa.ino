...
uint8_t data[16]; //ARREGLO DE RECEPCION RF (16 BYTES POR DESENCRIPTACION AES)
uint8_t envio[16]; //ARREGLO DE TRANSMISION LoRa (16 BYTES POR ENCRIPTACION AES)

//LLAVES PARA DESENCRIPTACION Y ENCRIPTACION AES POR RF Y LoRa
...
void loop()
{
   if (radio.available())
   {
      ...
      aes128_dec_single(key, data); //DESENCRIPTACION DE DATOS POR AES EN RF
      if(data[2] == 1 || data[2] == 2 || data[2] == 3 || data[2] == 4 || data[2] == 5)
      {
        ...
        aes128_enc_single(keyLoRa, envio); //ENCRIPTACION DE DATOS POR AES EN LoRa
